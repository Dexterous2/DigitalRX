...
const PharmacyTable = ({ head, data, yHeight, tableHeading, func, isDot, filterOrder, setFilterOrder }) => {
    ...
    return (
        <div className='bg-white-color  box_shadow_sec rounded-xl w-full overflow-hidden '>
            <div className='flex py-4 px-8 justify-between items-center text-primary-color pb-4 sm:flex-row flex-col sm:gap-0 gap-4'>
                ...
                <div className='flex'>
                    <span className='flex items-center gap-2 border-2 border-primary-color px-2 rounded-xl'>
                        ...
                        <Field type='text' placeHolder="Search" bgColor="bg-white-color" onChange={(e) => setFilterOrder(e.target.value)} value={filterOrder} />
                    </span>
                    {isDot &&
                        <ThreeDots func={func} />
                    }
            ...
            <div className={`w-full max-[1400px]:overflow-x-auto max-[650px]:w-[90vw] max-[450px]:w-[88vw] ${yHeight ? `${yHeight} overflow-y-auto` : ""} `}>
                {data?.length <= 0 ? <div className='flex items-center justify-center h-[100%] text-gray-400'>
                    ...
                    <table className='w-full max-[1400px]:w-[46rem] max-[1024px]:w-full max-[650px]:w-[50rem] '>
                            ...
                            <tr >
                                {tableHead?.map((item, i) => (
                                    <React.Fragment key={i + 1}>
                                        <th className={`text-primary-color ${item?.isStart ? "text-start" : "text-center"} py-2  
                                ${i === 0 ? "pl-8" : "pl-0"}
                                ${i === tableHead?.length - 1 ? "pr-8" : "pr-0"}`} key={i + 1}>
                                            {item?.head}
                                        </th>
                                    </React.Fragment>
                                ))}
                        ...
                        <tbody>
                            {data?.map((item, i) => (
                                <tr key={item.id ? item.id : i} className='tableRow'>
                                    {item && Object.keys(item).map((key, j) => (
                                        <React.Fragment key={j + 3}>
                                            {

                                                (key !== 'id' && key !== "status") &&
                                                <td className={`py-2 ${j === 1 ? "pl-8 text-start" : "pl-0 text-center"}`}>
                                                    {item[key]}
                                                </td>
                                            }
                                            {
